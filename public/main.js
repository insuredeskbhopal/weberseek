...
function initWeberseekApp() {
  ...
  // =========================================================================
  // 0. MOBILE & PREFERENCE DETECTION (mobile optimization layer)
  //     Desktop code paths are untouched: these flags only reduce work on
  //     phones (DPR cap, particle density, skipped 3D system, paused loops).
  // =========================================================================
  ...
  function initDisintegrationEngine() {
    ...
    // Cap the backing-store scale on phones: 3x DPR full-screen canvases
    // cost ~4x fill rate for no visible gain on small glassmorphic particles.
    dScale = IS_MOBILE
    ...
    if (!animationFrameId) {
      if (REDUCED_MOTION) {
        // Reduced motion: draw one static frame of the title; the scroll
        // cross-fade in handleScroll() still fades the whole intro layer.
        time = 0;
        currentScrollProgress = 0;
        renderStaticTitleFrame();
      } else {
  ...
  // Single paint of the resting title (used for prefers-reduced-motion).
  function renderStaticTitleFrame() {
    if (!dCtx || particles.length === 0) return;
    dCtx.clearRect(0, 0, dWidth, dHeight);
    for (let i = 0; i < particles.length; i++) {
      const p = particles[i];
      dCtx.fillStyle = `${p.color} ${p.alpha})`;
      dCtx.fillRect(p.ox, p.oy, p.size, p.size);
    }
  }
  ...
  function generateTextParticles() {
    ...
    // Mobile sampling step: fewer particles on small screens (identical look,
    // fraction of the per-frame cost). Desktop keeps the original step of 2.
  ...
  function renderDisintegrateLoop() {
    // Pause the loop while the tab is hidden (rAF is throttled anyway; this
    // keeps the frame budget at zero instead of one wasted callback).
    if (document.hidden) {
      animationFrameId = requestAnimationFrame(renderDisintegrateLoop);
      return;
    }

    // Mobile: once the intro is fully disintegrated there is nothing left to
    // paint, so skip the full-DPR clearRect + particle loop entirely and just
    // keep a cheap idle frame scheduled. Desktop keeps the original behavior.
    if (IS_MOBILE && currentScrollProgress >= 0.98) {
      animationFrameId = requestAnimationFrame(renderDisintegrateLoop);
      return;
    }
  ...
  // Window resize: regenerate particles only when the WIDTH changes.
  // iOS Safari fires resize while the URL bar collapses on every scroll;
  // regenerating the full particle field each time would jank scrolling.
  ...
  if (mobileToggle && navLinks) {
    ...
    mobileToggle.addEventListener('click', () => {
      setDrawerState(!navLinks.classList.contains('active'));
    });
    ...
    // Close when tapping outside the drawer/header on mobile
  ...
  if (ambientCanvas) {
    ...
    const count = IS_MOBILE
      ? Math.min(Math.floor(window.innerWidth / 55), 18)  // fewer, cheaper particles on phones
      : Math.min(Math.floor(window.innerWidth / 28), 45);
    ...
    function renderAmbientParticles() {
      ...
      if (document.hidden) {
        ambientRafId = requestAnimationFrame(renderAmbientParticles);
        return;
      }
    ...
    if (REDUCED_MOTION) {
      // Static: paint one frame of slowly drifting dots, no loop.
      ambientParticles.forEach(p => {
        aCtx.beginPath();
        aCtx.arc(p.x, p.y, p.radius, 0, Math.PI * 2);
        aCtx.fillStyle = `${p.color} ${p.alpha})`;
...
Sent via WeberSeek Official Website`;
      ...
      fetch('/api/lead', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: clientName, email: clientEmail, company: companyName, serviceType: projectTypeEl ? projectTypeEl.value : 'single_page', details: projectDetails }) }).catch(function(e){ console.error(e); });
  ...
  function initSolarSystemOrbit() {
    ...
    // On phones the 720px 3D orbit is replaced by a static, legible tech
    // grid (see buildMobileTechGrid). Desktop keeps the orbit engine.
    if (IS_MOBILE) {
      buildMobileTechGrid();
      return;
    }
    ...
    // Pause the orbit loop when the section scrolls out of view (saves the
    // per-frame transform work of 8 planets while it is offscreen).
    if ('IntersectionObserver' in window) {
      const solarObserver = new IntersectionObserver((entries) => {
        solarVisible = entries[0].isIntersecting;
      }, { rootMargin: '120px 0px' });
      solarObserver.observe(solarWrapper);
    }
    ...
    function renderPlanets() {
      if (document.hidden || !solarVisible) {
  ...
  // Build a legible static tech-stack grid for phones, reusing the planet
  // data already in the DOM (icon + name + role). Hidden on desktop by CSS.
  function buildMobileTechGrid() {
    const grid = document.getElementById('mobileTechGrid');
    if (!grid) return;
    ...
    planets.forEach((planet) => {
      const iconBox = planet.querySelector('.planet-icon-box');
      const name = planet.querySelector('.planet-name');
      const role = planet.querySelector('.planet-role');

      const item = document.createElement('div');
      item.className = 'mobile-tech-item';

      if (iconBox) {
        const iconClone = iconBox.cloneNode(true);
        iconClone.classList.add('planet-icon-box');
      ...
      if (role) {
        ...
        roleSpan.textContent = role.textContent;
  ...
  // Graceful degradation for the media assets that ship separately. When a
  // referenced image/video is missing, remove the broken node so the dark
  // glass surfaces keep looking intentional instead of showing broken glyphs.
  function initMediaFallbacks() {
    document.querySelectorAll('.coverflow-card-bg').forEach((img) => {
      img.addEventListener('error', () => { img.style.display = 'none'; }, { once: true });
    });

    document.querySelectorAll('.peeker-character img').forEach((img) => {
      img.addEventListener('error', () => {
        const peeker = img.closest('.peeker-character');
        if (peeker) peeker.style.display = 'none';
      }, { once: true });
    });

    const video = document.querySelector('.contact-webm-video');
    if (video) {
      video.addEventListener('error', () => {
        const frame = video.closest('.contact-video-frame');
        if (frame) frame.style.display = 'none';
      }, { once: true });

      // iOS Safari cannot play WebM; hide the frame instead of showing an
      // empty player (the glass frame's own styling keeps the layout intact).
      const canWebm = video.canPlayType('video/webm');
      if (!canWebm || canWebm === 'no') {
        const frame = video.closest('.contact-video-frame');
        if (frame) frame.style.display = 'none';
      }
    }
  ...
  // Initialize coverflow, scroll animations, big card parallax & solar system
...
if (typeof window !== 'undefined') {
  window.initWeberseekApp = initWeberseekApp;
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initWeberseekApp);
  } else {
    // DOM already loaded in Next.js
    setTimeout(initWeberseekApp, 50);
  }
}
